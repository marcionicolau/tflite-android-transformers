...
class LLMNextWord(application: Application) : AndroidViewModel(application) {
    ...
    fun promptToken() {
        nextTokenJob = viewModelScope.launch {
            ...
            getNextWord(_prompt.value!!, 3)
        }

    }

    suspend fun getNextWord(startPrompt: String, quantityOfTokens: Int = 3) =
...
fun TextView.formatCompletion(prompt: String): Unit {
    text = when {
        prompt.isEmpty() -> "When I was a child, I"
            ...
            val bgCompletionColor = androidx.core.content.res.ResourcesCompat.getColor(resources, co.huggingface.android_transformers.gpt2.R.color.colorPrimary, context.theme)
